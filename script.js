...
const CORRECT_PIN     = "030406";               // ← Her birthday DDMMYY
...
function startHeartTree() {
  ...
  const W = cvs.width, H = cvs.height, CX = W / 2;
  const GROUND      = H * .74;
  const TRUNK_MAX_H = Math.min(H * .44, 280);
  const TRUNK_W     = 32;
  ...
  const HEART_CX = CX, HEART_CY = GROUND - TRUNK_MAX_H + 70, HS = Math.min(W * .36, 140);
  ...
  function loop(now) {
    ...
    if (!swipeDone && el > T.swipeOut) {
      ...
      cvs.style.transform  = 'translateX(28%)';
